...
// Live mode: point VITE_API_BASE_URL at the web deployment's gateway proxy
// (https://<vercel-app>/api/gateway) — the ADM/ERH proxy URLs are derived —
// or at a direct gateway origin with VITE_ADM_API_BASE_URL /
// VITE_ERH_API_BASE_URL set explicitly.
const env = (import.meta as unknown as { env?: Record<string, string | undefined> }).env ?? {};
const apiBaseURL = env.VITE_API_BASE_URL ?? "";
const apiKey = env.VITE_API_KEY ?? "dev-key";
const derived = deriveServiceEndpoints(apiBaseURL);
const admBaseURL = env.VITE_ADM_API_BASE_URL ?? derived.adm;
const erhBaseURL = env.VITE_ERH_API_BASE_URL ?? derived.erh;
...
function renderAssessment(useCase: PublicServiceUseCase, assessment: TrustAssessment): string {
  return `
    ...
    <section class="evidence-grid" id="live-api-status">
      <article>
        <h2>Live API Status</h2>
        <p>${
          apiBaseURL
            ? "Checking the deployed gateway and engines..."
            : "Offline demo mode. Set VITE_API_BASE_URL (and VITE_API_KEY) to probe the deployed APIs."
        }</p>
      </article>
    </section>
  `;
...
function statusRow(label: string, ok: boolean, detail: string): string {
  return `
    <article class="signal-row">
      <span class="status-dot status-${ok ? "ready" : "missing"}" aria-label="${ok ? "ok" : "failing"}"></span>
      <div>
        <strong>${label}</strong>
        <p>${detail}</p>
      </div>
...
async function renderLiveStatus(): Promise<void> {
  if (!apiBaseURL) return;
  const container = appRoot.querySelector<HTMLElement>("#live-api-status article");
  if (!container) return;

  const [gatewayResults, engineResults] = await Promise.all([
    probeGateway({ baseURL: apiBaseURL, apiKey }, selectedUseCase).catch(() => []),
    admBaseURL || erhBaseURL
      ? probeEngines({ admBaseURL, erhBaseURL }, selectedUseCase).catch(() => [])
      : Promise.resolve([]),
  ]);

  const rows = [
    ...gatewayResults.map((r) => statusRow(r.label, r.ok, r.detail)),
    ...engineResults.map((r) => statusRow(r.label, r.ok, r.detail)),
  ];
  container.innerHTML = `
    <h2>Live API Status</h2>
    <div class="signal-list">${rows.join("") || "<p>No probes configured.</p>"}</div>
  `;
}
